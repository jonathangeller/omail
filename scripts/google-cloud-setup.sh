...
Usage: google-cloud-setup.sh [--project ID] [--no-open]
  ...
  --project ID   Cloud project to create or reuse. Default: omail-<random>
...
if [[ -z $project_id ]]; then
  project_id="omail-$(tr -dc 'a-z0-9' </dev/urandom | head -c 6)"
fi
...
if gcloud projects describe "$project_id" >/dev/null 2>&1; then
  ...
  gcloud projects create "$project_id" --name="Omail"
fi
